...
export const ShareBox: Component<{ post: CompleteNotePost }> = (props) => {
  ...
  return (
    <div>
      ...
      <HStack>
        <A
          href={`https://twitter.com/intent/tweet?url=https://aivy.run/notes/${
            props.post.id
        ...
        <A
          href={`http://line.me/R/msg/text/?${encodeURIComponent(
            `${props.post.title} | ${props.post.profiles.username} https://aivy.run/notes/${props.post.id}`,
          )}`}
        >
          <IconButton>
      ...
      >
        ...
        <IconLink
          ...
          `}
          onClick={() => {
            navigator.clipboard.writeText(`https://aivy.run/notes/${props.post.id}`)
            toast({
              title: 'クリップボードにコピーしました。',
              description: 'ノートのリンクをクリップボードにコピーしました。',
              status: 'success',
              isClosable: true,
            })
          }}
